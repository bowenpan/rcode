...
for(i in 1:nfolds){
  ...
  model.nb = buildModel("response", predictors=predictor.names, training, type ='classify', method = 'randomForest')
  pred.nb  = genPred(model.nb, newdata=validation, method='randomForest')
...
cat("Average AUC of randomForest = ", mean(nb.aucs), " with standard error = ", sd(nb.aucs)/sqrt(nfolds), "\n" )
...
our.test.nb  = genPred(model.nb, newdata=our.test.binned, method='randomForest')
...
if(submission.model == "model.ensemble")
{
  ...
  ensemble.final = data.frame(logreg = probs.logreg, tree=  probs.tree, nb = probs.nb) #####START FROM HERE
