...
#predictors$Previous_Rate <- (predictors$Previous_Rate/100)
# 
# for(i in 1:nrow(predictors)){
#   
#   if(is.na(predictors$Previous_Rate[i]) == FALSE)
#   {
#     if (predictors$rate[i] >= (predictors$Previous_Rate[i]/100)) {
#       predictors$higherRate[i] = 1
#     } else if(predictors$rate[i] < (predictors$Previous_Rate[i]/100)) {
#       predictors$higherRate[i] = 0
#     }
#     else {predictors$higherRate[i] = 0}
#   } else {predictors$higherRate[i] = 'NA'}
# }
...
for(i in 1:nfolds){
  ...
  predictor.names = c('rel_compet_rate','Amount_Approved','mp','creditRatio', 'Competition_rate')
  ...
  model.svm = buildModel("response", predictors=predictor.names, training, type ='classify', method = 'linearSVM')
  pred.svm  = genPred(model.svm, newdata=validation, method='linearSVM')
  
  combined.preds = (pred.logreg + pred.nb)/2
  combined.preds = (pred.logreg + pred.nb + pred.svm)/3
...
our.test.svm  = genPred(model.svm, newdata=our.test.binned, method='linearSVM')
...
# Use these if you only want logreg and nb in the ensemble
# ensemble.training = data.frame(logreg = pred.logreg, nb = pred.nb, response = validation$response)
# ensemble.our.test = data.frame(logreg = our.test.logreg, nb = our.test.nb)

# Use these if you want logreg, nb, and SVM in the ensemble
ensemble.training = data.frame(logreg = pred.logreg, nb = pred.nb, svm = pred.svm, response = validation$response)
ensemble.our.test = data.frame(logreg = our.test.logreg, nb = our.test.nb, svm = our.test.svm)
...
if(submission.model == "model.ensemble")
{
  ...
  #probs.svm = genPred(model.svm, newdata = test.binned, method = "linearSVM")
  
  ensemble.final = data.frame(logreg = probs.logreg, nb=  probs.nb)
...
}else if(submission.model == "model.svm"){
  probabilities = genPred(model.svm, newdata = test.binned, method = "logisticRegression")
} else {
